...
public class GrayImageThresholdInverted extends Thread {
    ...
    private Mat binary;
    private Mat binaryOtsu;


    private double slidersRate = 100.0;
    private ImageCanvas canvas;

    public GrayImageThresholdInverted() {
        this.start();
    }

    public GrayImageThresholdInverted(String path) {
        IMAGE_PATH = path;
        this.start();    }
    ...
    public void run() {
        try {
            ...
            binaryOtsu = originalGrayImage.clone();
            Imgproc.threshold(
                    originalGrayImage,                               //source
                    binary,                           //destination
                    0,
                    255,                                //maxValue//ADAPTIVE_THRESH_MEAN_C
                    Imgproc.THRESH_BINARY_INV               //THRESH_BINARY, THRESH_BINARY_INV, THRESH_MASK, THRESH_OTSU, THRESH_TOZERO, THRESH_TOZERO_INV, THRESH_TRIANGLE, THRESH_TRUNC
            );
            Imgproc.threshold(
                    binary,                               //source
                    binaryOtsu,                           //destination
                    64,
                    175,                                //maxValue//ADAPTIVE_THRESH_MEAN_C
            ...
            Imgcodecs.imencode(".jpg", binary, matOfByte);
